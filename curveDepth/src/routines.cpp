...
//   parMode: 
//     < -0.5: hyperplane;
//     > 0.5: parabolic surface;
//     > 1.5: parabolic surface with point for which we compute depth 
//            on the border.
...
double empDistPortion(double* empDistPrj, double* empDistWeights,
                      ...
                      double* refEmpDistPt, EmpDist &theEmpDist, 
                      double* dir, int d, double parConst1, double parConst2, 
                      int parMode){
  // Sum positive projections weighting them
  double posProb = 0;
  double* tmpX = new double[d]; // Point with reference in the origin
  for (int i = 0; i < nVoxels; i++){ // for each voxel of the 'empDist':
    if (parMode < -0.5){ // simple hyperplane
      if (empDistPrj[i] - pointPrj >= - eps){ // if it is in a positive halfspace
        posProb += empDistWeights[i]; // then add its weight
      }
    }else{ // a more complex surface
      if (parMode > 1.5){ // point for which the depth is computed on the border
        // Calculate point with reference in the origin, 
        // absolute value and the projection on 'dir'
        double tmpPrj = 0;
        double tmpAbs = 0;
        for (int j = 0; j < d; j++){
          tmpX[j] = theEmpDist.vals[i][j] - 
            (dir[j] * parConst2 / (1 + parConst1)) - refEmpDistPt[j];
          tmpAbs+= pow(tmpX[j], 2);
          tmpPrj+= tmpX[j] * dir[j];
        }
        tmpAbs = sqrt(tmpAbs);
        // Check belonging of the point to the subspace
        double checkFun = tmpAbs - parConst1 * tmpPrj;
        if  (checkFun <= parConst2 + eps){
        // if ((empDistPrj[i] - pointPrj >= - eps) && 
        //     (checkFun <= parConst2 + eps)){
          posProb += empDistWeights[i]; // then add its weight
        }
      }else{ // point for which the depth is computed in the center
        // Calculate point with reference in the origin, 
        // absolute value and the projection on 'dir'
        double tmpPrj = 0;
        double tmpAbs = 0;
        for (int j = 0; j < d; j++){
          tmpX[j] = theEmpDist.vals[i][j] - refEmpDistPt[j];
          tmpAbs+= pow(tmpX[j], 2);
          tmpPrj+= tmpX[j] * dir[j];
        }
        tmpAbs = sqrt(tmpAbs);
...
double approxOneDepth(EmpDist &refEmpDist, EmpDist &curEmpDist,
                      ...
                      double parConst1, double parConst2, int parMode){
