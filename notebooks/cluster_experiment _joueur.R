...
# ==============================================================================
  ...
  save(partition_finale, file = "partition_finale.RData")
  
  
  # 1. Créer un tableau comparatif avec le nom des joueurs
comparaison <- data.frame(
  CAH = cluster_assignments,
  DDClust = partition_relabel
)

# 2. Afficher la matrice de confusion (vue globale)
table(CAH = comparaison$CAH, DDClust = comparaison$DDClust)

# 3. Lister uniquement les joueurs qui ont changé de cluster
joueurs_differents <- comparaison[comparaison$CAH != comparaison$DDClust, ]
print(joueurs_differents)
  ...
  DDClust <- function(points,
    ...
    # Vérification : chaque cluster doit contenir au moins 2 joueurs
    while (length(table(labels)) < nClusters ||
           min(table(labels)) < 2) {
    ...
    while (TRUE) {
      ...
      # ----------------------------------------------------------
      # Joueurs potentiellement mal classés
      # ----------------------------------------------------------
      ...
      while (length(indicesWrong) > 0) {
        ...
        indicesWrong <- indicesWrong[
          -which(indicesWrong %in% curIndices)
        ]
        
        # Nouvelle partition
        newLabels <- labels
        
        newLabels[curIndices] <-
          nearestClust[curIndices]
    ...
    crit_final <- mean(res_final$ReDs)
    
    cat("\nPartition finale - K =", nClusters, "\n")
    print(table(labels))
    
    cat("Critère final C =", crit_final, "\n")
    ...
    return(list(
      ...
      crit     = crit_final,
      ReDs     = res_final$ReDs,
      pureReDs = res_final$pureReDs,
      pureSils = res_final$pureSils,
      depths   = res_final$depths,
      iter     = iter - 1
    ))
  }
  ...
  for (idx in seq_along(Ks)) {
    ...
    resultats_K$crit[idx] <-
      res_K[[idx]]$crit
    
    resultats_K$iterations[idx] <-
      res_K[[idx]]$iter
  }
