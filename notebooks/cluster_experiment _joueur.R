...
worker.dist <- function(inp, ...) {
  library(spatstat)
  args   <- list(...)[[1]]
  curves <- args$curves   # liste de ppp
  sigma  <- args$sigma
  
  ppp_i <- curves[[inp$iRows]]
  ppp_j <- curves[[inp$iCols]]
...
dist.matchs.parallel <- function(curves, sigma,
                                  nproc = 2, cltype = "SOCK") {
  
  parInput <- list(curves = curves, sigma = sigma)
