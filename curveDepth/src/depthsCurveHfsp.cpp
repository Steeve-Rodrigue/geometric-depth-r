...
NumericVector depthCurveTukey(List objects, List data, int nDirs = 100,
  ...
  // Rcout << "Rf_isNull(subsamples) = " << Rf_isNull(subsamples) << std::endl;
  // Rcout << "Rf_isNull(objects) = " << Rf_isNull(objects) << std::endl;
  // Rcout << "subsamples.isNotNull" << subsamples.isNotNull() << std::endl;
  ...
  for (int i = 0; i < m; i++){
    ...
    /*
    Rcout << "curEmpDist.n = " << curEmpDist.n << std::endl;
    ...
    // 3.b) Subsample points of the object if necessary:
    if (subs){
      ...
      if (subsamples.isNotNull()){ // whether subsamples indicated in input
        //Rcout << "Entered branch of subsamples indicated in input." << std::endl;
        NumericVector refSubsample = as<List>(subsamples)(i);
...
//    return depths;
    ...
    if (exactEst){
      ...
      depths(i) = approxOneDepth(refEmpDist, curEmpDist, genEmpDist, dirs,
             nDirs, d, minMassObj, minMassDat, parConst1, parConst2, parMode);
    }
  }
...
NumericVector depthCTukey(List objects, List data, int nDirs = 100,
  ...
  return depthCurveTukey(tmpObj, tmpDat, nDirs, subs = false, fracInt, fracEst, 
                         0, 1, -1, R_NilValue, exactEst, minMassObj, minMassDat);
}
